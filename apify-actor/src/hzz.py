...
def get_hzz_category_groups(category: str) -> list[str]:
    """Return the (static) subgroup labels for an HZZ category.

    Accepts either a category key (e.g. ``construction``) or its Croatian
    label. The subgroup lists were discovered once from the HZZ landing page
    (see ``discover_subgroups.py``) and are stable, so we keep them static
    instead of re-discovering on every run.
    """
    if not category:
        return []
    candidate = category.strip()
    if candidate in HZZ_CATEGORY_GROUPS:
        return list(HZZ_CATEGORY_GROUPS[candidate])

    # Fall back to resolving a Croatian label / slug back to its key.
    ...
    for key, label in HZZ_CATEGORIES.items():
        if candidate_slug in {key, _slugify_category(label)}:
            return list(HZZ_CATEGORY_GROUPS.get(key, []))
    return []


def hzz_group_to_category() -> dict[str, str]:
    """Reverse map: subgroup label -> owning category key."""
    return {
        group: key
